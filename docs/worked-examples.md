...
| [Theme development](../skills/shopify-theme-development/references/worked-example.md) | Keep selected price, variant ID and availability aligned | Synthetic Chromium DOM tests and separate Shopify Theme Check |
...
Browser dependencies are separate from the lightweight packaging dependencies:
...
The harness uses the real example JavaScript but generated HTML rather than Shopify-rendered Liquid. It sends no Shopify requests and does not submit an order. Static Liquid/JSON checking is separate:
...
The [evaluation manifest](../evals/worked-examples.json) intentionally keeps independent model evaluation at `not_run`. Authored examples and deterministic checks must not silently become model scores.
